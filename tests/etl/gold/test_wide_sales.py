...
def test_join_master_tables(spark):
    '''
    Positive test that the funtion _join_master_tables returns the expected DataFrame.
    '''
    ...
    customers_schema = st.StructType([
        st.StructField('CustomerKey', st.IntegerType(), True),
        st.StructField('Name', st.StringType(), True),
    ])
    customers_master_test = spark.createDataFrame(customers_master_test_data, schema=customers_schema)
    ...
    products_master_test = spark.createDataFrame(products_master_test_data, schema=products_schema)
...
def test_get_wide_sales(mock_join_master_tables, spark, some_df):
    '''
    Docstring for test_get_wide_sales

    :param mock_join_master_tables: Description
